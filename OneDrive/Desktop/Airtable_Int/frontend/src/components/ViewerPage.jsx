...
const ViewerPage = () => {
  ...
  useEffect(() => {
    axios
      ...
      .catch((err) => setStatus({ message: "Could not load form configuration.", type: "error" }))
  ...
  const isFieldVisible = useCallback(
    (field) => {
      ...
      const controllerField = config.fields.find((f) => f.airtableFieldId === field.conditional.showIfField)
  ...
  const handleConfirmSubmit = () => {
    const submissionData = Object.keys(formData).reduce((acc, key) => {
      ...
      if (field && isFieldVisible(field) && (formData[key] || typeof formData[key] === "boolean")) {
    ...
    axios
      ...
      .catch((err) => setStatus({ message: "Error submitting form.", type: "error" }))
  }
    ...
    return (
      <div className="container">
        ...
        <div className="preview-list">
          {config.fields
            .filter(
              (field) => isFieldVisible(field) && (formData[field.name] || typeof formData[field.name] === "boolean"),
            )
            .map((field) => (
        ...
        <div className="button-group" style={{ marginTop: "20px" }}>
          <button onClick={() => setViewMode("form")} style={{ marginRight: "10px", backgroundColor: "#6c757d" }}>
            Edit
          </button>
